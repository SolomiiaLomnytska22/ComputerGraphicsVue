    ...
    <div class = "main-content">
     ...
     <div class = "content">
         <div class="toolbar-btn">
            ...
            <v-btn class="btn" variant="flat" color="#D73246" @click="saveImage">Save
         ...
         <div class = "work-area">
            <div class = "picture-container"> 
                <canvas ref= "imageCanvas" alt="Your Image"  @mousemove="getPixelColor"></canvas>
                    <div class="toolbar">
                        <v-btn class="btn" variant="text" color="white"  @mousedown="buttonPressed = true" @mouseup="buttonPressed = false" >
                            <svg-icon style = "color: #ffffff" type="mdi" :path="mdiEyeOff"></svg-icon>
                        </v-btn>
                        <span class="info"><h3 style="display:inline">HSV:</h3> {{ formattedHSV  }} </span>
                        <span class="info"><h3 style="display:inline">CMYK:</h3> {{ formattedCMYK }}</span>
                    </div>
    ...
    formattedCMYK() {
     ...
     methods: {
       ...
       saveImage(){
        ...
        canvas.toBlob((blob)=> {saveAs(blob, this.uploadedFileName);});
       },
    ...
    adjustSaturationAndBrightness() {
        ...
        for (let i = 0; i < originalCanvas.height; ++i) {
            ...
            if(hsv[0]*360<200||hsv[0]*360>260){
                hsv.s += this.saturationValue;
                hsv.v += this.brightnessValue;
